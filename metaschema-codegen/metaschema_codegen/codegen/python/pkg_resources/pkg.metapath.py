...
class Phrase:
    ...
    def __eq__(self, value):
        if isinstance(value, str):
            return self.name == value
        return (
            isinstance(value, Phrase) or isinstance(value, Lex)
...
class GLR:
    ...
    def eval(self, stackelem):
        ...
        if (
            len(self.input) == 0
            and len(self.workspace) == 1
            and self.workspace[0] == "S"
        ):
        ...
        for rule in self.autmtn.possibleReductions():
            ...
            if valid:
                newstate = self.stack.add(
                    automatonRollback["parent"], self.compileState()
                )
                self.nodesToEval.push(newstate)
    ...
    def compileState(self):
        return (
            ...
            [item for item in self.input],
            self.autmtn.getState(),
        )
...
class Language:
    def __init__(self, lexicon=None, grammar=None, interpreter=None):
        ...
        self.ready = (
            lexicon is not None and grammar is not None and interpreter is not None
        )
    ...
    def ror(
        self, fr, tos
    ):  # expand fr into *any* of to (i.e., the tos are all or'd together)
        for to in tos:
