...
class JsonParser:
    ...
    def _parse_raw_data(
        self, input: typing.Union[dict, list, str]
    ) -> typing.Any:  # TODO - fix type annotations
        """
        ...
        if isinstance(input, dict):
            ...
            for key in input.keys():
                ...
                if isinstance(input[key], dict):
                    ...
                    parsed_input.extend(
                        self._parse_raw_list(name=key, input=input[key])
                    )
                    ...
                    raise IOError(
                        "Invalid data passed to raw parser. This is a super-secret exception condition you should never see!"
                    )
            return parsed_input
        else:
            raise IOError(
    ...
    def _parse_raw_list(
        self, name: str, input: list[typing.Any]
    ) -> list[dict[str, str]]:
        """
        ...
        for element in input:
            parsed_element = {}
            parsed_element["group-as-name"] = name
