...
from ..core.schemaparse import MetaSchemaSet, MetaSchema
...
class PackageGenerator:
    ...
    def __init__(
        self, parsed_metaschemas: MetaSchemaSet, destination_directory: Path
    ) -> None:
    ...
    def __init__(self, metaschema: MetaSchema, global_refs: list[GlobalRef]) -> None:
        ...
        for assembly in self.metaschema.get("define-assembly", []):
            self.class_generators.append(
                self._generate_assembly(assembly_dict=assembly)
            )
...
class DatatypeGenerator:
    """
    A class to convert the Metaschema Datatypes into referencable classes
    """
